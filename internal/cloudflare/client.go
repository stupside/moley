...
func NewClient(cfg *config.MoleyConfig) (*CloudflareClient, error) {
	...
	if token == "" {
		return nil, fmt.Errorf("cloudflare.api_token must be set in moley.yml. Please create an API token in your Cloudflare dashboard with Zone:Read and DNS:Edit permissions")
	}
