...
func NewAccessService(client *cfgo.Client, accountID string, config *Config) *accessService {
...
func (s *accessService) CreateApplication(ctx context.Context, params ports.AccessApplicationParams) (string, error) {
	...
	if sessionDuration == "" {
		sessionDuration = domain.DefaultSessionDuration.String()
	}
